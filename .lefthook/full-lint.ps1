...
Write-Host '[CHECK] Running full project lint...' -ForegroundColor Cyan
        ...
        if ($eslintTool -or (Test-Path "node_modules/.bin/eslint")) {
            npx eslint src --max-warnings 10 2>&1 | ForEach-Object {
                if ($_ -match "error|Error") {
                    ...
                    Write-Host "  [INFO] $_" -ForegroundColor $infoColor
                }
            }
        } else {
            Write-Host "  [SKIP] eslint not installed (optional)" -ForegroundColor "Yellow"
        }
    }
    Pop-Location
}
...
if ((Get-Command cargo -ErrorAction SilentlyContinue) -and (Test-Path "Cargo.toml")) {
    ...
    Write-Host "  [SKIP] Cargo not available or no Cargo.toml found" -ForegroundColor "Yellow"
}
...
if ((Get-Command cargo -ErrorAction SilentlyContinue)) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "  [FAIL] Rust formatting issues found" -ForegroundColor $errorColor
        Write-Host "    Run: cargo fmt --all" -ForegroundColor "Yellow"
        $hasErrors = $true
    } else {
        Write-Host "  [OK] Rust code is properly formatted" -ForegroundColor $successColor
    }
}
        ...
        if (Test-Path "package.json") {
            ...
            if ($prettierTool -or (Test-Path "node_modules/.bin/prettier")) {
                ...
                if ($LASTEXITCODE -eq 0) {
                    Write-Host "  [FAIL] Format issues found in $dir" -ForegroundColor $errorColor
                    Write-Host "    Run: npx prettier --write src/**/*.{ts,tsx,js}" -ForegroundColor "Yellow"
                    $hasErrors = $true
                } else {
                    Write-Host "  [OK] Node code is properly formatted in $dir" -ForegroundColor $successColor
                }
            }
        }
        Pop-Location
...
if ($hasErrors) {
    Write-Host '[FAIL] Linting issues found! Fix issues before pushing.' -ForegroundColor $errorColor
    Write-Host ''
    ...
    Write-Host '[OK] All lint checks passed!' -ForegroundColor $successColor
    exit 0
}
