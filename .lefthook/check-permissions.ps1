...
Write-Host '[CHECK] Checking file permissions and hygiene...' -ForegroundColor Cyan
...
foreach ($file in $stagedFiles) {
    ...
    foreach ($pattern in $forbiddenPatterns) {
        if ($file -match $pattern) {
            Write-Host "  [WARN] Problematic file: $file" -ForegroundColor Yellow
            $foundIssues = $true
        }
    }
    ...
    if ($file -match "\.\(bat|cmd|ps1|exe\)$") {
        Write-Host "[INFO] Executable script: $file (make sure this is intentional)" -ForegroundColor Cyan
    }
}

if ($foundIssues) {
    Write-Host ''
    Write-Host '[WARN] FILE HYGIENE CHECKS: Issues detected' -ForegroundColor Yellow
    Write-Host ''
    Write-Host 'Some files that should not be committed were detected:' -ForegroundColor Yellow
    Write-Host ''
...
Write-Host '[OK] File hygiene checks passed' -ForegroundColor Green
exit 0
