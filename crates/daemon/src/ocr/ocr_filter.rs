...
pub fn find_main_content_column(
    words: &[OcrWord],
    page_width: usize,
    page_height: usize
) -> (f32, f32) {
...
fn cluster_into_lines(mut words: Vec<OcrWord>) -> Vec<LineInfo> {
    // STEP 1: Sort ONLY by Y (Top to Bottom).
    // This is a valid Total Order. It will never panic.
    words.sort_by(|a, b| a.y.total_cmp(&b.y));

    let mut lines = Vec::new();
    let mut current_line_words: Vec<OcrWord> = Vec::new();

    // We need to track the 'visual center' of the current line to decide if new words belong to it.
    // Initializing with the first word's Y is usually safer than hardcoding -100.
    let mut current_line_y = if let Some(w) = words.first() { w.y } else { 0.0 };
    let mut current_line_h = if let Some(w) = words.first() { w.height } else { 0.0 };

    for word in words {
        // Calculate dynamic threshold based on the current line's height
        let threshold = (current_line_h * 0.5).max(10.0);

        // Check if word belongs to the current line (is it "close enough" vertically?)
        if !current_line_words.is_empty() && (word.y - current_line_y).abs() > threshold {
            // STEP 2: The line is finished. NOW sort it by X (Left to Right).
            // This is safe because we are only sorting a small, related group.
            current_line_words.sort_by(|a, b| a.x.total_cmp(&b.x));
            ...
            // Start a new line group using this word as the anchor
            current_line_y = word.y;
            current_line_h = word.height;
        }

        // If the buffer was empty (first word of new line), set the anchor Y
...
pub fn process_ocr_pipeline(
    raw_words: Vec<OcrWord>,
    page_width: usize,
    page_height: usize
) -> Vec<LineInfo> {
...
fn assemble_paragraphs(lines: &[LineInfo]) -> Vec<String> {
    ...
    for line in lines {
        ...
        let is_new_paragraph =
            vertical_gap > para_spacing_threshold ||
