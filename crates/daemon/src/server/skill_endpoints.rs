...
pub async fn semantic_search(
    ...
    // Serialize embedding to JSON for SQLite vec extension
    ...
    if let Some(ref filters) = payload.filters {
        if let Some(ref app_names) = filters.app_names {
            if !app_names.is_empty() {
                let apps_list: Vec<String> = app_names.iter().map(|a| format!("'{}'", a.replace("'", "''"))).collect();
                sql.push_str(&format!(" AND f.app_name IN ({})", apps_list.join(",")));
            }
        }
        
        if let Some(ref time_range) = filters.time_range {
            if let Some(ref start_time) = time_range.start {
                sql.push_str(&format!(" AND f.captured_at >= '{}'", start_time));
            }
            if let Some(ref end_time) = time_range.end {
                sql.push_str(&format!(" AND f.captured_at <= '{}'", end_time));
            }
        }
    }
    
    sql.push_str(&format!(" ORDER BY distance ASC LIMIT {}", limit));
        ...
        .await
    {
        Ok(rows) => {
            ...
            let results: Vec<SemanticSearchResult> = rows.iter().map(|row| {
...
pub async fn hybrid_search(
    ...
    info!("Hybrid search request: query={}, keywords={:?}", &payload.query, &payload.keywords);
    ...
    if let Some(ref filters) = payload.filters {
        if let Some(ref app_names) = filters.app_names {
            if !app_names.is_empty() {
                let apps_list: Vec<String> = app_names.iter().map(|a| format!("'{}'", a.replace("'", "''"))).collect();
                filter_conditions.push_str(&format!(" AND f.app_name IN ({})", apps_list.join(",")));
            }
        }
        
        if let Some(ref time_range) = filters.time_range {
            if let Some(ref start_time) = time_range.start {
                filter_conditions.push_str(&format!(" AND f.captured_at >= '{}'", start_time));
            }
            if let Some(ref end_time) = time_range.end {
                filter_conditions.push_str(&format!(" AND f.captured_at <= '{}'", end_time));
            }
        }
    }
    
    // Build FTS match query if keywords provided
    let fts_match = if !keywords.is_empty() {
        keywords.iter()
            .map(|k| format!("\"{}\"", k.replace("\"", "")))
            .collect::<Vec<_>>()
            .join(" OR ")
    } else {
        // Use query terms as keywords
        payload.query.split_whitespace()
            .filter(|w| w.len() > 2)
    ...
    // Combined hybrid search with RRF scoring
    let sql = format!(r#"
        WITH vector_results AS (
            SELECT 
                ...
                COALESCE(f.browser_url, '') as browser_url,
                c.text_content,
                1.0 - vec_distance_cosine(v.embedding, '{embedding_json}') as vector_score,
                'vector' as source
            FROM vec_chunks v
            JOIN chunks c ON v.chunk_id = c.id
            JOIN frames f ON c.frame_id = f.id
        ...
        fts_results AS (
            ...
            JOIN frames f ON c.frame_id = f.id
            WHERE chunks_fts MATCH '{fts_match}' {filter_conditions}
            ORDER BY fts_score DESC
            LIMIT {fetch_limit}
        ),
        combined AS (
            SELECT 
                chunk_id, frame_id, captured_at, app_name, window_title, browser_url, text_content,
                MAX(vector_score) as vector_score,
                MAX(fts_score) as fts_score,
                CASE 
                    WHEN MAX(vector_score) > 0 AND MAX(fts_score) > 0 THEN 'both'
                    WHEN MAX(vector_score) > 0 THEN 'vector'
                    ELSE 'fts'
                END as search_type,
                (COALESCE(MAX(vector_score), 0) * 0.6 + COALESCE(MAX(fts_score), 0) * 0.4) as combined_score
            FROM (
                SELECT *, CASE WHEN source = 'vector' THEN vector_score ELSE 0 END as vector_score,
                       CASE WHEN source = 'fts' THEN fts_score ELSE 0 END as fts_score
                FROM vector_results
                UNION ALL
                SELECT *, 0 as vector_score, fts_score
                FROM fts_results
            )
            GROUP BY chunk_id
        )
        ...
        .await
    {
        Ok(rows) => {
            ...
            let results: Vec<HybridSearchResult> = rows.iter().map(|row| {
                HybridSearchResult {
                    chunk_id: row.try_get("chunk_id").unwrap_or(0),
                    frame_id: row.try_get("frame_id").unwrap_or(0),
                    captured_at: row.try_get::<String, _>("captured_at").unwrap_or_default(),
