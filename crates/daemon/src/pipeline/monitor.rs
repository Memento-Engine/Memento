            ...
            ::spawn_blocking(
                move || -> Result<DynamicImage> {
                    if use_sck {
                        ...
                        monitor.capture_image().map_err(Error::from).map(DynamicImage::ImageRgba8)
                    }
                }
            ).await
        ...
        ::spawn_blocking(move || {
            match XcapMonitor::all() {
                Ok(monitors) => {
                    ...
                    tracing::debug!("Found {} monitors with IDs: {:?}", monitor_count, monitor_ids);
...
pub fn get_capture_backend() -> &'static str {
    if use_sck_rs() { "sck-rs (ScreenCaptureKit)" } else { "xcap (legacy)" }
}
...
pub async fn get_primary_monitor_id(
) -> Result<u32, Box<dyn std::error::Error + Send + Sync>> {

    let monitor_id = tokio::task::spawn_blocking(
        move || -> Result<u32, Box<dyn std::error::Error + Send + Sync>> {
            let monitors = Monitor::all()?;

            for monitor in monitors {
                if monitor.is_primary().unwrap_or(false) {
                    return Ok(monitor.id().unwrap_or_default());
                }
            }
